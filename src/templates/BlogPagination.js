...
const headerStyle =
  "pt-14 lg:pt-16 lg:pb-24 px-6 md:px-8 lg:px-12 xl:px-20 2xl:px-36 max-w-screen-2xl w-full mx-auto flex flex-col"
...
export default function BlogPage(props) {
  ...
  return (
    <div className="bg-trueGray-900">
      ...
      <SEO
        title="Kartavya's Blog"
        description="Farzad Mohebi's blog. A collection of writing focused on the current state of tech and web development tutorials."
      />
      <div className={classNames(headerStyle, "mb-12 md:mb-0")}>
        {isMobile ? null : (
          <form
            ...
            method="get"
            autoComplete="off"
            className={`lg:-mt-16 flex flex-row items-center ${
          ...
          >
            <label htmlFor="header-search" className="-mr-2 ml-2 absolute">
              <svg
                ...
                class="bi bi-search"
                viewBox="0 0 16 16"
                className={`${hasFocus ? "text-orange-500" : ""}`}
              >
        ...
        <div className={`flex ${currentPage === 1 ? "lg:pt-16" : ""}`}>
          {!hasFocus ? (
            <div className="max-w-3xl 2xl:max-w-4xl flex flex-col justify-center">
              ...
              {currentPage === 1 ? (
                <div
                  className="text-white text-opacity-80 font-semibold text-4xl pb-2 pt-3 font-mono md:min-w-keepWmd 2xl:min-w-keepWlg"
                  id="post-anchor"
                >
                  Latest Posts
                </div>
                ...
                <div
                  className="text-white text-opacity-80 font-semibold text-4xl pb-2 font-mono md:min-w-keepWmd 2xl:min-w-keepWlg"
                  id="post-anchor"
                >
                  More Posts
                </div>
              ...
              <div className="flex justify-start items-center">
                {true && (
                  <Link
                    to={isFirst ? "" : `${prevPage}`}
                    rel="prev"
                    className={`${isFirst ? "pointer-events-none" : ""}`}
                  >
                    <button
                      className={`bg-trueGray-800 px-3 py-1 flex rounded outline-none mr-1 ${
                        isFirst
                ...
                {true && (
                  ...
                  >
                    <button
                      className={`bg-trueGray-800 px-3 py-1 flex rounded outline-none ml-1 ${
                        isLast
                          ? "disabled:opacity-50 disabled:cursor-not-allowed"
                          : ""
                      }`}
            ...
            <div>
              <div className="flex flex-col">
                <div
                  className="text-white font-semibold text-4xl font-mono md:min-w-keepWmd 2xl:min-w-keepWlg"
                  id="post-anchor"
                >
                  Search Results{" "}
                </div>
                <span className="text-xs text-opacity-80 text-orange-500">
