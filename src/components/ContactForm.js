...
const validate = values => {
  ...
  if (values.firstName[0] === " ") {
    ...
    !/^[A-Za-z][A-Za-z'-]+([A-Za-z][A-Za-z'-]+)*/.test(values.firstName)
  ) {
    errors.firstName = "Invalid field entry"
  ...
  if (values.lastName[0] === " ") {
    ...
    !/^[A-Za-z][A-Za-z'-]+([A-Za-z][A-Za-z'-]+)*/.test(values.lastName)
  ) {
    errors.lastName = "Invalid field entry"
    ...
    >
      ...
      >
        <input
          type="hidden"
          name="apikey"
          value="53b27b90-2cca-4b3a-b90f-8dc3da879ea2"
        />
        <input
          type="hidden"
          name="subject"
      ...
      <div className="text-5xl text-center mb-16 mt-12  leading-normal font-extrabold tracking-tight text-gray-100">
        or simply call me!{" "}
        <span className="text-orange-500">+358 40 628 28 23</span>
      </div>
    </div>
  )
}
