  ...
  return (
    <div className={`max-h-screen ${isPost ? "sticky top-10" : "w-full ml-6"}`}>
      <div className={`${isPost ? "pt-5 2xl:pt-10" : "p-5"}`}>
        {!isPost ? (
          <div className="text-white text-opacity-80 font-mono text-3xl pt-6 flex flex-row items-start w-full">
            Share
          </div>
        ) : (
          <div className="text-white font-mono text-lg lg:text-xl 2xl:text-2xl">
            Share
          </div>
        )}
        <div className="flex flex-col items-start">
          <TwitterShareButton
            className="flex flex-row justify-center mt-5 outline-none"
            url={isPost ? url : "https://www.kartavyas.com/blog"}
            title={isPost ? title : "Farzad Mohebi's blog"}
            hashtags={isPost ? tags : []}
          >
            <svg
            ...
            <div className="text-white text-opacity-80 font-blogBody text-sm 2xl:text-base">
              Share on Twitter
            </div>
          </TwitterShareButton>
          <LinkedinShareButton
          ...
          <FacebookShareButton
            className="flex flex-row justify-center mt-5 outline-none"
            url={isPost ? url : "https://www.kartavyas.com/blog"}
            quote="Check out Kartavya's blog!"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width={`${iconDimen}`}
